...
document.querySelectorAll('.stat-card').forEach(card => {
  card.addEventListener('click', () => {
    ...
    // 취약점 페이지로 전환
    navItems.forEach(nav => nav.classList.remove('active'));
    document.querySelector('[data-page="vulnerabilities"]').classList.add('active');
    
    pages.forEach(p => p.classList.remove('active'));
    document.getElementById('vulnerabilities-page').classList.add('active');
    
    // 타이틀 업데이트
    document.getElementById('pageTitle').textContent = '취약점 분석';
    document.getElementById('pageSubtitle').textContent = `${card.querySelector('.stat-label').textContent} 심각도 문제 확인`;
